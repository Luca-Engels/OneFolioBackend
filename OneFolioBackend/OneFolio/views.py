...
def user_list(request):
    if request.method == 'GET':
        users = list(users_collection.find())
        print(users)
        return JsonResponse([obj_to_json(user) for user in users], safe=False)
...
def user_detail(request, user_id):
    if request.method == 'GET':
        ...
        if user:
            return JsonResponse(obj_to_json(user))
        else:
...
def investment_list(request):
    if request.method == 'GET':
        investments = list(investments_collection.find())
        print(investments)
        return JsonResponse([obj_to_json(investment) for investment in investments], safe=False)
    elif request.method == 'POST':
        data = json.loads(request.body)
...
def investment_detail(request, investment_id):
    if request.method == 'GET':
        ...
        if investment:
            return JsonResponse(obj_to_json(investment))
        else:
            return HttpResponse(status=404)
    elif request.method == 'PUT':
        data = json.loads(request.body)
